...
class SecurityScreenUpdateUserHandler(
    ...
    suspend fun handle(user: User, messageId: Long?, update: suspend () -> TrackingSecurity?): BotScreen {
        val updated = update() ?: return ErrorScreen(user.id, UiError.UnregisteredUserError)
